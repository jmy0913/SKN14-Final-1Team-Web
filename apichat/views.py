...
from .utils.llm import run_llm, title_llm, title_llm2, suggest_llm
...
def update_session_title_inline(session, all_messages):
    ...
    if first_user and (not session.title or session.title == "새로운 대화"):
        session.title = title_llm(first_user.content)
        print('initial_title type:', type(first_user.content))
    ...
    if len(user_msgs) == 3:
        ...
        final_title = title_llm2(session.title, transcript)
        print('refine_title_transcript_type:', type(transcript))
...
def chat(request):
    if request.method == "POST":
        try:
            ...
            print('제목 갱신에 필요한 메시지 내용:', all_msgs)
            print('제목 갱신 메세지의 타입:', type(all_msgs))

            update_session_title_inline(session, all_msgs)

            # 추천 질문 생성
            suggestions = suggest_llm(user_message, response, k=5)
